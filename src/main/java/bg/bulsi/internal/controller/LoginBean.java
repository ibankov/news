...
public class LoginBean implements Serializable {
	...
	public String login() {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(null, 
					new FacesMessage(FacesMessage.SEVERITY_INFO, rbBundle.getString("wrongInfo"), "fail"));
			e.printStackTrace();
		}
		username = "";
		password = "";
	...
	public String logout() {
		...
		try {
			...
			if (request.getRequestURL().toString().equals("http://10.10.0.113:8080/news/index.xhtml") 
					|| request.getRequestURL().toString().equals("http://localhost:8080/news/index.xhtml") 
					|| request.getRequestURL().toString().equals("http://127.0.0.1:8080/news/index.xhtml")) {
				return "login";
			}
			return "index";
		}
